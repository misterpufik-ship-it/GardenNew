...
ВАЖНО — сохранность позиций и фото:
• Блюда, цены, фото и описания визуального меню хранятся в базе данных CouchCMS, а не в файлах Git.
• Обычный деплой (обновление кода на сервере) НЕ должен удалять ваши позиции и загруженные фото.
• Не удаляйте строки в админке, если не хотите убрать блюдо с сайта — корзина в списке удаляет позицию из базы.
• Служебные скрипты в папке _maintenance/ (импорт, регистрация шаблонов, восстановление) могут перезаписать данные — их запускает только разработчик по запросу, не при каждом деплое.
...
Если после сохранения на сайте старый текст — сначала Ctrl+F5, затем сообщите разработчику о полной очистке кэша.
    ...
    <cms:editable name='group_deploy' label='Деплой и сохранность данных' type='group' collapsed='1' order='85' />
    <cms:editable name='guide_deploy' label='Содержание' group='group_deploy' type='textarea' order='86'><![CDATA[
Что хранится в базе (не в Git):
• Визуальное меню — все карточки (Кальяны, Кухня, Бар), описания, цены, ссылки на фото.
• Текстовое меню — все позиции и разделы.
• Галерея, акции, тексты страниц, настройки отступов гамбургер-меню и пр.

Обычный деплой:
• Обновляет только PHP/HTML/CSS/JS файлы на сервере.
• Не удаляет и не перезаписывает позиции меню, если вы сами их не меняли в админке.
• Кэш страниц очищается отдельно — при необходимости разработчик запускает clear-cache.

Что может затереть данные (только вручную, по запросу):
• import-visual-taplink-cli.php — полная замена списков визуального меню.
• register-*-web.php с параметрами migrate_legacy=1 или повторная миграция полей.
• restore-visual-menu-missing-web.php — только ДОБАВЛЯЕТ отсутствующие позиции, существующие не трогает.
• sync-visual-menu-descriptions-web.php — только заполняет поле «Описание» из текстового меню.

Правило для разработчиков:
• Никогда не запускать импорт/миграцию меню при рутинном деплое.
• Перед любым скриптом, меняющим меню, — согласование с владельцем сайта.
• Позиции и фото, добавленные вами в админке, считаются эталоном — код подстраивается под них, а не наоборот.
]]></cms:editable>
